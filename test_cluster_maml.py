...
parser.add_argument('--save_dir', type=str, default="model/maml_cluster/",
                    help='directory in which model should be saved')
parser.add_argument('--log_dir', type=str, default="log/maml_cluster/",
...
class TaskTailor:
    ...
    def add_sample(self, sample):
        ...
        #     sc.extend(sample[k])
        #     if k % 5 == 4:
        #         s.append(np.ravel(sc))
        #         sc = []
        self.samples.append(sample)
    def divide(self):
        results = []
        for i in range(100):
            result = self.model.predict([np.reshape(self.samples, (-1, 100, 20))])
            results.append(result.tolist())
            self.samples[99 - i] = np.zeros((20,))
        print(results)
        return np.argmax(results[0]), results
    def getlen(self):
...
def test(path, c, q):
    ...
    for agent in agents:
        agent.load_model(args.save_dir, c, episode)
        ...
        if i % args.action_interval == 0:
            ...
            for _ in range(args.action_interval):
                ...
                for agent_id, agent in enumerate(agents):
                    ...
                    if total_decision_num < agent.meta_test_start:
                        s = list(last_obs[agent_id])
                        s.append(last_phase[agent_id][0])
                        s.append(actions[agent_id])
                        s.append(rewards[agent_id])
                        for ob in obs[agent_id]:
                            s.append(ob)
                        s.append(env.world.intersections[agent_id].current_phase)
                        samples.append(s)
                    elif total_decision_num == agent.meta_test_start:
                        for agent in agents:
                            agent.load_model(args.save_dir, tailor.divide(), episode)
                    total_decision_num += 1
                if total_decision_num <= 100:
                    tailor.add_sample(samples)
