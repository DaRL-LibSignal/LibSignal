...
class MADDPGAgent(RLAgent):
    ...
    def train(self):
        ...
        target_q = 0.0
        for ag in self.agents:
            sample_index = random.sample(list(range(ag.buffer_size)), self.batch_size)
        ...
        if self.local_q_learn:
            q_input_target = torch.cat((b_tp_list[self.rank], full_action_tp), dim=1)
        ...
        p_prob = self.G_softmax_forward(p)
        p_reg = torch.mean(torch.square(p_prob))
        p_loss = -torch.mean(self.q_model(q_input, train=False))
    ...
    def load_model(self, e):
        ...
        self.target_q_model = self._build_model(self.q_lenght, 1)
        self.target_p_model = self._build_model(self.ob_length, self.action_space.n)
        self.target_q_model.load_state_dict(torch.load(model_q_name))
        self.target_p_model.load_state_dict(torch.load(model_p_name))
    ...
    def save_model(self, e):
        ...
        torch.save(self.target_p_model.state_dict(), model_p_name)
        torch.save(self.target_q_model.state_dict(), model_q_name)
