...
class DQNAgent(RLAgent):
    def __init__(self, action_space, ob_generator, reward_generator, iid):
        ...
        ob_length = [State.dims["D_" + feature_name.upper()] for feature_name in self.list_feature_name]
    ...
    def choose(self, ob):
        ...
        act_values = self.model.farward(ob)
        return np.argmax(act_values[0])
    ...
    def _encode_sample(self, minibatch):
        ...
        actions = torch.from_numpy(actions_t).float()
        rewards = torch.from_numpy(rewards_t).float()
    ...
    def replay(self):
        ...
        target = rewards + self.gamma * np.amax(out, axis=1)
