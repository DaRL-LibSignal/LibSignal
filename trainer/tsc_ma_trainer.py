...
@Registry.register_trainer("tsc-ma")
class TSCMATrainer(BaseTrainer):
    '''
    ...
    def create_metrics(self):
        ...
        #TODO: interact with env directly
        agents = [ag for ag in self.agents.values()]
        agents.sort(key=lambda x: x.rank)
        self.metric = Metrics(lane_metrics, world_metrics, self.world, agents)
    ...
    def create_agents(self):
        ...
        '''
        self.agents = {}
        # TODO: flexible num_agent
        num_agent = len(self.world.intersections)
        for i in range(0, num_agent):
            tmp_ag = Registry.mapping['model_mapping'][Registry.mapping['command_mapping']['setting'].param['agent']](self.world, i)
            self.agents.update( {tmp_ag.id:tmp_ag})
