...
parser.add_argument('--save_dir', type=str, default="model/meta_intellight", help='directory in which model should be saved')
parser.add_argument('--log_dir', type=str, default="log/meta_intellight", help='directory in which logs should be saved')
        ...
        while i < args.steps:
            ...
            for n in range(len(path)):
                if i % args.action_interval == 0:
                    ...
                    for agent_id, agent in enumerate(meta_agents[0]):
                        agent.remember(meta_ob[n], 1 - (actions[0]==meta_last_actions[n][0]), rewards[0], obs[0])
                        meta_episodes_rewards[n][agent_id] += rewards[agent_id]
                        meta_episodes_decision_num[n] += 1
                        total_decision_num += 1

                    meta_last_obs[n] = obs
                    meta_last_actions[n] = actions
                if n < len(path) - 1:
                    i = j
            for agent_id, agent in enumerate(meta_agents[0]):
    ...
    for i in range(args.steps):
        if i % args.action_interval == 0:
            ...
            for _ in range(args.action_interval):
                ...
                rewards = np.mean(rewards_list, axis=0)
                for agent_id, agent in enumerate(agents):
                    agent.remember(last_obs[agent_id], actions[agent_id], rewards[agent_id],
                                   obs[agent_id])
                    total_decision_num += 1
                last_obs = obs
                for agent_id, agent in enumerate(agents):
