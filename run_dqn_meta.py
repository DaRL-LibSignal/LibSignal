...
parser.add_argument('--save_dir', type=str, default="model/meta_dqn_4x4", help='directory in which model should be saved')
parser.add_argument('--log_dir', type=str, default="log/meta_dqn_4x4", help='directory in which logs should be saved')
        ...
        while i < args.steps:
            ...
            for n in range(len(path)):
                if i % args.action_interval == 0:
                    ...
                    for agent_id, agent in enumerate(meta_agents[0]):
                        agent.remember(meta_last_obs[n][agent_id], actions[agent_id], rewards[agent_id], obs[agent_id])
