...
parser.add_argument('--save_dir', type=str, default="model/metalight_4x4/",
                    help='directory in which model should be saved')
parser.add_argument('--log_dir', type=str, default="log/metalight_4x4/",
...
def meta_train(path):
    ...
    for e in range(args.episodes):
        ...
        for agent in key_agent:
            agent.save_model(args.save_dir, 0, e)
        meta_last_obs = []
        ...
        for n in range(len(path)):
            for agent in meta_agents[n]:
                agent.load_model(args.save_dir, cluster=0, e=e)
        while i < args.steps:
            # print(i)
            j = i
            if i % args.action_interval == 0:
                ...
                for n in range(len(path)):
                    ...
                    i = j
                    for agent_id, agent in enumerate(meta_agents[n]):
                        if total_decision_num[n] > agent.learning_start and total_decision_num[n] % agent.update_model_freq == agent.update_model_freq - 1:
                            agent.replay()
                        if total_decision_num[n] > agent.learning_start and total_decision_num[n] % agent.update_target_model_freq == agent.update_target_model_freq - 1:
            ...
            for agent_id, agent in enumerate(key_agent):
                if key_num > agent.learning_start and key_num % agent.update_model_freq == agent.update_model_freq - 1:
                    agent.replay()
                if key_num > agent.learning_start and key_num % agent.update_target_model_freq == agent.update_target_model_freq - 1:
                    agent.update_target_network()
            if all(dones):
                break
        # logger.info("Step 1:")
