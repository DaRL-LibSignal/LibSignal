...
parser.add_argument('--save_dir', type=str, default="model/maml_4x4",
                    help='directory in which model should be saved')
parser.add_argument('--log_dir', type=str, default="log/maml_4x4",
    ...
    for e in range(args.episodes):
        ...
        for n in range(len(path)):
            for agent in meta_agents[n]:
                agent.load_model(args.save_dir, n % cluster_num, e)
            i = 0
            while i < args.steps:
...
if __name__ == '__main__':
    ...
    for n in range(len(real_flow_path)):
        ...
        t1 = test(real_flow_path[n], 0)
        t2 = meta_test(real_flow_path[n], 0)
    ...
    #         logger.info("Meta Test Env: %d" % n)
    #         t1 = test(fake_flow_path[n], 0)
    #         t2 = meta_test(fake_flow_path[n], 0)
    #         result.append(np.minimum(t1, t2))
    #     logger.info(
    #         "Meta Test Result, Max: {}, Min: {}, Mean: {}".format(np.max(result), np.min(result), np.mean(result)))
