...
parser.add_argument('--save_dir', type=str, default="model/maml_cluster_1x5_individual",
                    help='directory in which model should be saved')
parser.add_argument('--log_dir', type=str, default="log/maml_cluster_1x5_individual",
...
def meta_train(path, p_path):
    ...
    for i in range(len(path)):
        s = []
        for j in range(5):
            s.append(((i * 5) + j ) % cluster_num)
        env2cluster.append(s)
    for e in range(args.episodes):
        individual_sample = []
        for n in range(len(path)):
            sample = []
            for a in range(len(meta_agents[n])):
                sample.append([])
            individual_sample.append(sample)
        ...
        for n in range(len(path)):
            for a in range(len(meta_agents[n])):
                meta_agents[n][a].load_model(args.save_dir, env2cluster[n][a], e)
            i = 0
            while i < args.steps:
                if i % args.action_interval == 0:
                    ...
                    for agent_id, agent in enumerate(meta_agents[n]):
                        ...
                        if len(individual_sample[n][a_index]) < agent.meta_test_start:
                            ...
                            individual_sample[n][a_index].append(s)
                        a_index += 1
        ...
        while i < args.steps:
            ...
            if i % args.action_interval == 0:
                for n in range(len(path)):
                    ...
                    rewards = np.mean(rewards_list, axis=0)
                    for a in range(len(meta_agents[n])):
                    # for agent_id, agent in enumerate(key_agents[env2cluster[n][a_index]]):
                        # if total_decision_num[n] < agent.learning_start:
                        #     continue
                        agent_id = a
                        key_agents[env2cluster[n][a]][a].remember(meta_last_obs[n + len(path)][agent_id], last_phase[agent_id], actions[agent_id],
                                       rewards[agent_id],
                                       obs[agent_id],
                ...
                for c in range(cluster_num):
                    for agent_id, agent in enumerate(key_agents[c]):
                        if np.sum(key_num[c]) / cluster_num > agent.learning_start and key_num[c] % agent.update_model_freq == agent.update_model_freq - 1:
                            agent.replay()
                        if np.sum(key_num[c]) / cluster_num> agent.learning_start and key_num[c] % agent.update_target_model_freq == agent.update_target_model_freq - 1:
                            agent.update_target_network()
            if all(dones):
                break
        # logger.info("Step 1:")
        ...
        for n in range(len(path)):
            ...
            if e > cluster_update_start - cluster_update_rate:
                for a in range(len(meta_agents[n])):
                    accumulate_reward[n][a] += meta_episodes_rewards[n][meta_agents[n][a].iid] / meta_episodes_decision_num[n]
        if e > cluster_update_start and e % cluster_update_rate == cluster_update_rate - 1:
            ...
            for c in range(cluster_num):
                ...
                for n in range(len(path)):
                    for a in range(len(meta_agents[n])):
                        if env2cluster[n][a] == c:
                            result.append(accumulate_reward[n][a] / cluster_update_rate)
            ...
            for n in range(len(path)):
                for a in range(len(meta_agents[n])):
                    ...
                    min_cluster = -1
                    for c in range(cluster_num):
                        if min_dis > np.abs(accumulate_reward[n][a] / cluster_update_rate - cluster_center[c]):
                            min_dis = np.abs(accumulate_reward[n][a] / cluster_update_rate - cluster_center[c])
                            min_cluster = c
                    env2cluster[n][a] = min_cluster
                    accumulate_reward[n][a] = 0
            logger.info("===================================")
            logger.info("Update Cluster")
            logger.info(env2cluster)
            logger.info("===================================")
        # samples.append(sample)
        np.save(args.save_dir + '/samples_{}.npy'.format(e), individual_sample)
